...
> = {
  ...
  previewnet: {
    ...
    bridgeUrl: "https://bridge.previewnet.tezosx.nomadic-labs.com",
  },
};

export function normalizeTezosXNetwork(
  raw: string | undefined,
  fallback: TezosXNetworkName = "previewnet",
): TezosXNetworkName {
...
const NETWORK_MODAL_META: Record<
  TezosXNetworkName,
  { michelsonChainId: string | null }
> = {
  testnet: { michelsonChainId: null },
  previewnet: { michelsonChainId: "NetXY2oPPzkxUW1" },
};
...
export function buildNetworkInfoModalRows(
  ...
  const meta = NETWORK_MODAL_META[stack];
